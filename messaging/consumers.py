...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if action == "mark_read":
            ...
            await self.channel_layer.group_send(
                ...
                {"type": "messages_read", "user_id": self.user.id}
            )

        elif action == "send_message" or data.get("message"):
            message_text = data.get("message")
            if message_text:
                new_message = await self.save_message(self.user.id, self.conversation_id, message_text)
                await self.trigger_notification(self.conversation_id, self.user.id)
                await self.channel_layer.group_send(
                    self.room_group_name,
                    {
                        "type": "chat_message",
    ...
    async def messages_read(self, event):
        await self.send(text_data=json.dumps({"action": "messages_read", "user_id": event["user_id"]}))

    async def message_edited(self, event):
        await self.send(text_data=json.dumps({"action": "edit_message", "message_id": event["message_id"], "new_text": event["new_text"]}))
    ...
    def save_message(self, user_id, conversation_id, text):
        ...
        User = get_user_model() # yalnız burada istifadə edirik
    ...
    def set_messages_read(self, user_id, conversation_id):
        Message.objects.filter(conversation_id=conversation_id, read_at__isnull=True).exclude(sender_id=user_id).update(read_at=timezone.now())
    ...
    def trigger_notification(self, conversation_id, sender_id):
        ...
        if receiver:
            ...
            if cache.add(lock_key, "locked", timeout=3):
                send_delayed_notification.apply_async((conversation_id, receiver.id), countdown=3)